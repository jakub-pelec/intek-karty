...
export default async function DashboardPage() {
  ...
  return (
    <main className="flex flex-col items-center px-4 pt-2 pb-16 md:px-8 md:pt-8">
      <h1 className="mb-10 text-center font-[family-name:var(--font-cinzel)] text-[14px] font-medium tracking-[0.4em] text-[#d4b36a]/90 uppercase md:mb-12">
      ...
      <div className="relative mb-12 flex w-full max-w-7xl flex-col lg:mb-16 lg:grid lg:grid-cols-[minmax(0,1fr)_minmax(260px,380px)_minmax(0,1fr)] lg:gap-10 xl:gap-12">
        ...
        <section className="order-2 flex flex-col pt-10 lg:order-1 lg:pt-12">
          ...
          {recentDraws.length === 0 ? (
            <p className="text-center text-[19px] text-[#d7d3c8]/40 italic">{t("noneYet")}</p>
          ) : (
              ...
              {recentDraws.map((draw, index) => (
                ...
                >
                  <div className="flex min-w-0 items-center gap-4">
                    ...
                    <span className="truncate text-[20px] text-[#d7d3c8]/90 italic">
                      {String(draw.cardNumber).padStart(2, "0")} {draw.cardName}
                      {draw.signature ? (
                        <span className="ml-2 font-[family-name:var(--font-cinzel)] text-[10px] tracking-widest text-[#d4b36a]/70 not-italic uppercase">
                          {tCommon("signed")}
                        </span>
                      ) : null}
                    </span>
                  </div>
                  <span className="shrink-0 font-[family-name:var(--font-cinzel)] text-[11px] tracking-[0.1em] text-[#d7d3c8]/40 uppercase">
        ...
        <section className="relative order-1 mb-8 flex flex-col items-center lg:order-2 lg:mb-0">
          ...
          {latest ? (
            <div className="flex w-full flex-col items-center">
              ...
              <p className="mt-8 font-[family-name:var(--font-cinzel)] text-[14px] tracking-[0.25em] text-[#d7d3c8]/50 uppercase">
                {toRoman(latest.cardNumber)} / {toRoman(latestTotal)}
              </p>
              <h2 className="mt-3 text-center font-[family-name:var(--font-cormorant)] text-[38px] tracking-wider text-[#d7d3c8] italic md:text-[48px]">
                {latest.cardName}
              </h2>
              <span className="mt-3 font-[family-name:var(--font-cinzel)] text-[13px] font-semibold tracking-[0.3em] text-[#00e5ff] uppercase">
                {tRarity(latest.cardRarity)}
                {latest.holographic ? t("holoSuffix") : ""}
                {latest.signature ? t("signedSuffix") : ""}
              </span>
              <span className="mt-1 font-[family-name:var(--font-cinzel)] text-[11px] tracking-[0.1em] text-[#d7d3c8]/40 uppercase">
            ...
            <RelicFrame sealed className="w-full max-w-[380px]">
              <div className="flex h-full items-center justify-center px-8 text-center">
                <p className="font-[family-name:var(--font-cormorant)] text-[28px] text-[#d7d3c8]/40 italic">
                  {t("noVision")}
                </p>
              </div>
            </RelicFrame>
        ...
        <section className="order-3 flex flex-col pt-10 lg:pt-12">
          ...
          {recentAchievements.length === 0 ? (
            <p className="text-center text-[19px] text-[#d7d3c8]/40 italic">
              {t("noneUnlocked")}
            </p>
          ) : (
            <ul className="space-y-1">
              {recentAchievements.map((row) => (
                ...
                >
                  <span className="text-center font-[family-name:var(--font-cinzel)] text-[15px] tracking-widest text-[#d7d3c8]/90 uppercase">
                    {row.name}
                  </span>
                  <span className="font-[family-name:var(--font-cinzel)] text-[11px] tracking-[0.2em] text-[#d7d3c8]/30 uppercase">
      ...
      <div className="relic-plinth mx-auto grid w-full max-w-6xl grid-cols-3 gap-3 px-3 pt-8 pb-4 md:gap-8 md:px-8 md:pt-10 md:pb-6">
        ...
        >
          <span className="relative font-[family-name:var(--font-cinzel)] text-[24px] text-[#d7d3c8] transition-colors group-hover:text-[#d4b36a] md:text-[28px]">
            {relics.owned} / {relics.total}
          </span>
          <span className="relative mt-2 max-w-full font-[family-name:var(--font-cinzel)] text-[11px] leading-[16px] tracking-[0.12em] text-[#d7d3c8]/40 uppercase transition-colors group-hover:text-[#d4b36a]/80 md:mt-3 md:text-[12px] md:tracking-[0.15em]">
